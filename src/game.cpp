...
Game::Game(GLFWwindow* window) : renderer_(window, camera_) {
  glfwSetWindowUserPointer(
      window,
      this);  // Current Game instance is set as the user pointer, enabling the
              // Game class to retrieve its instance in callbacks

  glfwSetMouseButtonCallback(
      window, mouse_button_callback);  // Triggered when a mouse button is
                                       // pressed or released
  glfwSetCursorPosCallback(
      window, mouse_move_callback);  // Triggered when the mouse cursor moves
  glfwSetScrollCallback(
      window,
      mouse_scroll_callback);  // Triggered when the scroll wheel is used

  glfwSetKeyCallback(
      window,
      key_callback);  // Triggered when a key is pressed, released, or repeated

  // sets camera position black or white side with 50/50 probability
  ...
  // Makes it so camera is still for a split second before game starts
...
#define CHECK(expr) \
  ...
  // Loads shaders and models from \resources\. If anything fails to load, game
  // will terminate
  // clang-format off
...
#undef CHECK
  ...
  float lag = 0.0f;  // Used to accumulate the time difference between frames,
                     // ensuring that game updates happen at a fixed rate (even
                     // if the rendering is faster)
  last_frame_ = static_cast<float>(
      glfwGetTime());  // Stores the timestamp of the last frame
  while (glfwWindowShouldClose(renderer_.get_window()) !=
         1) {  // Main game loop, runs as long as the window hasn't been closed
    const auto current_frame =
        static_cast<float>(glfwGetTime());  // Current time
    delta_time_ = current_frame - last_frame_;
    last_frame_ = current_frame;
    lag += delta_time_;

    glfwPollEvents();  // Processes any window events (keyboard/mouse actions)
                       // that have occurred since the last frame

    process_input();  // Processes user input (check if they pressed Esc and
                      // close the game if they did)
    while (lag >= k_ms_per_update) {
      // Ensures that the game logic happens at a fixed time step
      update();
      lag -= k_ms_per_update;
    }
...
void Game::update() {
  ...
  if (game_over_ || ai_.is_thinking()) {
    // No logic update is needed if game over or AI deciding its move
    return;
  }

  // Commence AI moving their piece
...
void Game::process_input() const {
  // If window isn't yet closed and user pressed Esc - close window
...
/// <summary>
/// Returns name of the given Piece
/// </summary>
