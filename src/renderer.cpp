...
void Renderer::install_shader(std::string_view name) {
  // Checks if given shader program exists
  const auto it{shader_map_.find(name)};
  assert(it != shader_map_.end());

  // Retrieves associated with given name ID and switches to it, if it wasn't
  // already in use
...
void Renderer::begin_picking(PickingMode picking_mode) {
  switch (picking_mode) {
    case PickingMode::Read:
      // Binds the picking framebuffer object (FBO) (picking_fbo_) as the read
      // framebuffer GL_READ_FRAMEBUFFER: A framebuffer specifically used for
      // reading pixel data
    ...
    case PickingMode::Write:
      ...
      constexpr std::array<GLint, 4> clear_color{
          -1, -1, -1, -1};  // Ensures that the background or areas with no
                            // objects are marked with -1 in the picking buffer,
                            // indicating "no selection"
      glClearBufferiv(
          GL_COLOR, 0,
          clear_color
              .data());  // Clears the color attachment (0) of the picking FBO
      constexpr GLfloat clear_depth{
          1.0F};  //  Resets the depth buffer to its maximum value, ensuring
                  //  that objects closer to the camera will overwrite farther
                  //  objects when rendered
      glClearBufferfv(
          GL_DEPTH, 0,
          &clear_depth);  // Clears the depth buffer of the picking FBO
      break;
  }
}
  ...
  switch (picking_mode) {
    case PickingMode::Read:
      glBindFramebuffer(GL_READ_FRAMEBUFFER,
                        0);  // Unbinds the picking framebuffer used for
                             // reading, 0 parameter specifies the default
                             // framebuffer (typically the screen framebuffer)
      break;
    case PickingMode::Write:
      glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);
      glFlush();  // Ensures that all issued OpenGL commands are sent to the GPU
                  // for execution
      glFinish();  // Blocks the application until all previously issued OpenGL
                   // commands are fully executed by the GPU
      break;
  }
}
