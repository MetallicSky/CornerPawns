...
class Renderer {
 public:
  ...
  /// <summary>
  /// Activates the OpenGL shader program identified by the given name
  /// </summary>
  ...
  /// <summary>
  /// Sets up OpenGL to allow reading or writing data from the picking FBO later
  /// </summary>
  /// <param name="picking_mode"> Can be either PickingMode::Read or
  /// PickingMode::Write</param>
  void begin_picking(PickingMode picking_mode);
  /// <summary>
  /// Finalizes the picking process by unbinding the framebuffer and ensures
  /// that all drawing operations are completed by flushing and finishing OpenGL
  /// commands
  /// </summary>
  /// <param name="picking_mode">Can be either PickingMode::Read or
  /// PickingMode::Write</param>
